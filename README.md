...
- If a **generic** type meets the unmanaged constraint and the case has `UseManagedStorage` set to true, the unmanaged generic type will share sorage with the non-generic unmanaged types. The `UnmanagedStorageSize` argument to the `UnionStorage` attribute must be explicitly set to a non-zero value or compilation will fail.
